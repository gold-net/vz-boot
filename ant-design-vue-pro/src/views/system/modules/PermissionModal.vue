    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          v-show="show"
          hasFeedback >
          <a-input placeholder="请输入菜单名称" v-decorator="[ 'title', validatorRules.title]" :readOnly="disableSubmit"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          :label="menuLabel"
          hasFeedback >
          <a-input placeholder="请输入组件名称" v-decorator="[ 'name', validatorRules.name]" :readOnly="disableSubmit"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="打开方式">
          <a-switch checkedChildren="外部" unCheckedChildren="内部" v-model="external"/>
        </a-form-item>

      </a-form>

      <!-- 选择图标 -->
      <icons-modal @choose="handleIconChoose" v-model="model.icon" @close="handleIconCancel" :iconChooseVisible="iconChooseVisible"></icons-modal>
    </a-spin>
...
<script>
  import {addPermission, duplicateCheck, editPermission, queryTreeList} from '@/api/api'
  ...
  export default {
    ...
    components: {IconsModal, JDictSelectTag},
    data () {
      return {
        drawerWidth:700,
        treeData:[],
    ...
    computed:{
      validatorRules:function() {
        return {
          title:{rules: [{ required: this.show, message: '请输入菜单标题!' }]},
    ...
    methods: {
      ...
      edit (record) {
        ...
        if(record.external!=null){
          this.external = record.external?true:false;
        }else{
          this.external = false;
        }

        this.show = record.menuType==2?false:true;
        this.menuLabel = record.menuType==2?'按钮/权限':'组件名称';
        ...
        this.visible = true;
        this.loadTree();
        let fieldsVal = pick(this.model, 'name', 'title', 'perms','permsType','component','url','sortNo','menuType','status', 'redirect');
        this.$nextTick(() => {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            this.model.external = this.external;
