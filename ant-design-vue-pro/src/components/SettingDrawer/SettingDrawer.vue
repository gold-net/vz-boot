...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onMultiTab(checked) {
        ...
        this.changeSetting('multipage', checked)
      },
      handleMenuTheme(theme) {
        this.$store.dispatch('ToggleTheme', theme)
        this.changeSetting('navTheme', theme)
      },
      doCopy() {
  ...
  production: process.env.NODE_ENV === 'production' && process.env.VUE_APP_PREVIEW !== 'true'
      ...
      handleLayout(mode) {
        ...
        this.changeSetting('layout', mode)
      },
      handleContentWidthChange(type) {
        this.$store.dispatch('ToggleContentWidth', type)
        this.changeSetting('contentWidth', type)
      },
      changeColor(color) {
        if (this.primaryColor !== color) {
          this.$store.dispatch('ToggleColor', color)
          updateTheme(color)
          this.changeSetting('color', color)
        }
      },
      handleFixedHeader(fixed) {
        this.$store.dispatch('ToggleFixedHeader', fixed)
        this.changeSetting('fixedHeader', fixed)
      },
      handleFixedHeaderHidden(autoHidden) {
        this.$store.dispatch('ToggleFixedHeaderHidden', autoHidden)
        this.changeSetting('autoHideHeader', autoHidden)
      },
      handleFixSiderbar(fixed) {
        if (this.layoutMode === 'topmenu') {
          this.$store.dispatch('ToggleFixSiderbar', false)
          this.changeSetting('fixSiderbar', false)
          return
        }
        this.$store.dispatch('ToggleFixSiderbar', fixed)
        this.changeSetting('fixSiderbar', fixed)
      },
