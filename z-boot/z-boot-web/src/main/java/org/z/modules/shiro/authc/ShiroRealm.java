...
public class ShiroRealm extends AuthorizingRealm {
	...
	private boolean jwtTokenRefresh(String token, String userName, String passWord) {
		String cacheToken = (String) cacheManager.get(CommonConstant.PREFIX_USER_TOKEN + token);
		if (StringUtils.isNotEmpty(cacheToken)) {
			// 校验token有效性
			if (!JwtUtil.verify(cacheToken, userName, passWord)) {
				String newAuthorization = JwtUtil.sign(userName, passWord);
				// 设置超时时间
				cacheManager.set(CommonConstant.PREFIX_USER_TOKEN + token, newAuthorization, JwtUtil.EXPIRE_TIME);
